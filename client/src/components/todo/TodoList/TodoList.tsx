...
const emptyTodo: ITodo = { id: "", type: "todo", text: "", complete: false };

const Todo = (props: IPropsTodos) => {
    const [upTodo, setUpTodo] = useState<ITodo>(emptyTodo);
    const { todos, updateTodo, deleteTodo } = props;

    const submitUpdateTodo = ({ id, type, text, complete }: ITodo) => {
        updateTodo({ id, type, text, complete });
        setUpTodo(emptyTodo);
    };
    ...
    return upTodo.id
        ...
        : <div className='todo-list'>
            <ul className="list">{todos.map(({ id, type, text, complete }) =>
                <li className="task" key={id}>
                    <div className={`task-container ${complete ? "complete" : ""}`}>
                        <span className="icon" onClick={() => submitUpdateTodo({ id, type, text, complete: complete ? false : true })}>
                        ...
                        <div className="icons">
                            <span className="icon" onClick={() => setUpTodo({ id, type, text, complete })}>
